...
add_shortcode('adminz_breadcrumb',function ($atts) {
        ...
        if($integrations=="wpseo"){
            ...
            if (is_single()) {
                ...
                if (is_tax()) {
                    ...
                    echo '<li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem" class="item item-current"><span itemprop="name">' . post_type_archive_title() . '</span><meta itemprop="position" content="'.do_shortcode($position).'" /></li>';
                    $position++;
                }
            } else if (is_page()) {
                // Standard page
    ...
    echo '</div> ';
    ?>
    <style type="text/css"> #<?php echo do_shortcode($_id); ?> {display: flex; margin: 5px 0; padding: 0; list-style: none; flex-wrap: wrap; } #<?php echo do_shortcode($_id); ?> li {margin: 0px; } #<?php echo do_shortcode($_id); ?> li.seperator{margin-right: 5px; margin-left: 5px; } #<?php echo do_shortcode($_id); ?> .item {text-decoration: none; } #<?php echo do_shortcode($_id); ?> .item-current {} </style> <?php
